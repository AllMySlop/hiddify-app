...
# The app and core are maintained as sibling repositories.
CORE_DIR ?= ..$(SEP)hiddify-core
BINDIR=$(CORE_DIR)$(SEP)bin
DESKTOP_OUT=$(CORE_DIR)$(SEP)bin
...
generate_kotlin_protos: 
	...
	# 	$(shell find $(CORE_DIR)/v2 $(CORE_DIR)/extension -name "*.proto")
	rsync -av --delete \
		--include='*/' \
		--include='*.proto' \
		--exclude='*' \
		$(CORE_DIR)$(SEP)v2 $(CORE_DIR)$(SEP)extension ./android/app/src/main/protos/
...
generate_go_protoc:
	make -C $(CORE_DIR) -f Makefile protos
	echo "SED: $(SED)"
generate_dart_protoc:
	mkdir -p lib/hiddifycore/generated
	protoc --dart_out=grpc:lib/hiddifycore/generated --proto_path=$(CORE_DIR)$(SEP)  $(shell find $(CORE_DIR)$(SEP)v2 $(CORE_DIR)$(SEP)extension -name "*.proto") 	google/protobuf/timestamp.proto ; \
...
build-headers:
	make -C $(CORE_DIR) -f Makefile headers && mv $(BINDIR)/$(CORE_NAME)-headers.h $(BINDIR)/hiddify-core.h

build-android-libs:
	make -C $(CORE_DIR) -f Makefile android 
	mv $(BINDIR)/$(LIB_NAME).aar $(ANDROID_OUT)/

build-windows-libs:
	make -C $(CORE_DIR) -f Makefile windows-amd64

build-linux-libs:
	make -C $(CORE_DIR) -f Makefile linux-amd64 

build-macos-libs:
	make -C $(CORE_DIR) -f Makefile macos

build-ios-libs: 
	rm -rf $(IOS_OUT)/HiddifyCore.xcframework 
	make -C $(CORE_DIR) -f Makefile ios  
